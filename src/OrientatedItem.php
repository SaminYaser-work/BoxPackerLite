...
class OrientatedItem implements JsonSerializable {
	 ...
	 */
	protected static array $stabilityCache = array();
	...
	public function __construct( Item $item, int $width, int $length, int $depth ) {
		...
		$this->dimensionsAsArray = array( $width, $length, $depth );
	...
	public function isSameDimensions( Item $item ): bool {
		...
		$itemDimensions = array( $item->getWidth(), $item->getLength(), $item->getDepth() );
		sort( $itemDimensions );
