...
class OrientatedItemFactory {
	...
	public function getPossibleOrientations(
		...
		if ( $item instanceof ConstrainedPlacementItem && ! $this->box instanceof WorkingVolume ) {
			$orientations = array_filter(
				$orientations,
				function ( OrientatedItem $i ) use ( $x, $y, $z, $prevPackedItemList ): bool {
	...
	protected function getUsableOrientations(
		...
		$stableOrientations = $unstableOrientations = array();
	...
	private function generatePermutations( Item $item, ?OrientatedItem $prevItem ): array {
		...
		if ( $prevItem && $prevItem->isSameDimensions( $item ) ) {
			return array( array( $prevItem->getWidth(), $prevItem->getLength(), $prevItem->getDepth() ) );
		}
		...
		// simple 2D rotation
		$permutations[ $w . '|' . $l . '|' . $d ] = array( $w, $l, $d );
		$permutations[ $l . '|' . $w . '|' . $d ] = array( $l, $w, $d );

		// add 3D rotation if we're allowed
		if ( ! $item->getKeepFlat() ) {
			$permutations[ $w . '|' . $d . '|' . $l ] = array( $w, $d, $l );
			$permutations[ $l . '|' . $d . '|' . $w ] = array( $l, $d, $w );
			$permutations[ $d . '|' . $w . '|' . $l ] = array( $d, $w, $l );
			$permutations[ $d . '|' . $l . '|' . $w ] = array( $d, $l, $w );
		}
